...
class RegisterDoseUseCase:
    """Registra uma dose (tomada, pulada ou supervisionada).

    Regras de negócio:
    - Paciente só pode autoregistrar doses não supervisionadas do próprio tratamento ativo.
    - Dose supervisionada deve ser registrada por profissional (registered_by != null).
    - Profissional de outra unidade não pode registrar doses no tratamento.
    ...
    async def execute(
        ...
        if actor_role == "patient":
            registered_by = await self._validate_patient_access(
